...
def index(request):
    rawcart = Cartt.objects.filter(user=request.user)
    for item in rawcart:
...
def placeorder(request):
    if request.method == "POST":
        ...
        trackno = 'hajar'+str(random.randint(1111111,9999999))
        while Orderr.objects.filter(tracking_no = trackno) is None :
            trackno = 'hajar'+str(random.randint(1111111,9999999))
